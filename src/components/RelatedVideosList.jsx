...
const RelatedVideosList = () => {
  ...
  const VideoCard = React.memo(({ video }) => {
    return (
      <div className="bg-white border border-green-100 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow video-card flex">
        <div className="relative w-[45%] bg-gray-100" style={{ aspectRatio: '1/1' }}>
          {video.thumbnailPath ? (
                <img 
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent flex items-end p-2">
            <div className="text-white text-xs bg-black/60 px-1.5 py-0.5 rounded">
              播放量: {video.viewCount}
            </div>
          </div>
        </div>
        <div className="p-3 w-[55%]">
          <h3 className="font-bold text-gray-800 mb-1 line-clamp-2 text-sm">{video.title}</h3>
          <p className="text-xs text-gray-600 mb-2 line-clamp-3">{video.description}</p>
          <div className="flex justify-between items-center text-xs text-gray-500">
            <span className="text-[10px]">发布于: {new Date(video.createdAt).toLocaleDateString('zh-CN')}</span>
  ...
  const fetchVideos = useCallback(async () => {
    ...
    try {
      ...
      if (response && response.code === 200) {
        ...
        // Only update hasMore if we're not on the last page
  ...
  useEffect(() => {
    // Don't fetch on initial load (currentPage 1 is handled by the first useEffect)
    if (currentPage > 1) {
      fetchVideos();
    }
  ...
  return (
    <div className="sticky top-4 max-h-[calc(100vh-6rem)] overflow-y-auto">
      <div className="mb-10">
        ...
        {videos.length > 0 ? (
          <div>
            <div ref={videosGridRef} className="grid grid-cols-1 gap-6">
